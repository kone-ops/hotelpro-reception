...
use Database\Seeders\PredefinedFormFieldsSeeder;
use Illuminate\Console\Command;

class InitializeFormFieldsCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'fields:init 
                            {--hotel= : ID spécifique de l\'hôtel} 
                            {--force : Réinitialiser les champs existants}';
     ...
     * @var string
     */
    protected $description = 'Initialiser les champs de formulaire prédéfinis pour tous les hôtels ou un hôtel spécifique';
    ...
    {
        $this->info('🚀 Initialisation des champs de formulaire...');
        $this->newLine();

        $hotelId = $this->option('hotel');
        $force = $this->option('force');

        // Si un hôtel spécifique est spécifié
        if ($hotelId) {
            $hotel = Hotel::find($hotelId);
            
            if (!$hotel) {
                $this->error("❌ Hôtel avec ID {$hotelId} non trouvé.");
                return 1;
            }

            $this->initializeHotel($hotel, $force);
            return 0;
        }

        // Sinon, initialiser tous les hôtels
        $hotels = Hotel::all();

        if ($hotels->isEmpty()) {
            $this->warn('⚠️  Aucun hôtel trouvé dans la base de données.');
            return 1;
        }
        ...
        foreach ($hotels as $hotel) {
            $this->initializeHotel($hotel, $force, false);
            $progressBar->advance();
        }
    ...
    /**
     * Initialiser les champs pour un hôtel donné
     */
    private function initializeHotel(Hotel $hotel, bool $force = false, bool $verbose = true)
    {
        $existingCount = $hotel->formFields()->count();

        if ($existingCount > 0 && !$force) {
            if ($verbose) {
                $this->warn("⚠️  L'hôtel '{$hotel->name}' a déjà {$existingCount} champs.");
                $this->info("   Utilisez --force pour réinitialiser.");
            }
            return;
        }
        ...
        if ($existingCount > 0 && $force) {
            if ($verbose) {
                $this->comment("🔄 Suppression des {$existingCount} champs existants pour '{$hotel->name}'...");
            }
            $hotel->formFields()->delete();
        }

        // Créer les champs prédéfinis
        ...
        if ($verbose) {
            $this->info("✓ {count($fields)} champs créés pour '{$hotel->name}'");
        }
    }

    /**
     * Retourne les champs prédéfinis (même logique que le seeder)
     */
