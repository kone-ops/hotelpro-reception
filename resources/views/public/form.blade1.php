...
<body data-hotel-id="{{ $hotel->id }}">
                    ...
                    <div class="section-card">
                        ...
                        <div id="groupeFields" class="conditional-field">
                            <div class="alert alert-info mb-3">
                                ...
                                Vous avez choisi une enregistrement de <strong>groupe</strong>. Veuillez renseigner les informations ci-dessous.
                            </div>
                            <div class="row">
                    ...
                    <div class="section-card">
                        ...
                        <div id="uploadMethodSection" class="conditional-field mb-4">
                            ...
                            <div class="upload-method-buttons">
                                <div class="upload-method-btn active" id="uploadMethodBtn">
                        ...
                        <!-- Upload Files -->
                        @if($formConfig->isVisible('piece_identite_recto') || $formConfig->isVisible('piece_identite_verso'))
                        <div id="uploadFilesSection" class="conditional-field show">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="file" name="piece_identite_recto" id="fileRecto" class="form-control" accept="image/jpeg,image/jpg,image/png,image/webp,application/pdf,.jpg,.jpeg" {{ $formConfig->getRequiredAttribute('piece_identite_recto') }}>
                                    <small class="text-muted">JPG, JPEG, PNG, WEBP ou PDF - Max 5MB</small>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="file" name="piece_identite_verso" id="fileVerso" class="form-control" accept="image/jpeg,image/jpg,image/png,image/webp,application/pdf,.jpg,.jpeg" {{ $formConfig->getRequiredAttribute('piece_identite_verso') }}>
                                    <small class="text-muted">JPG, JPEG, PNG, WEBP ou PDF - Max 5MB</small>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Fonction pour gérer l'attribut 'required' selon la section visible.
            // IMPORTANT : ne jamais mettre 'disabled' sur les inputs file — un champ disabled
            // n'est pas envoyé avec le formulaire, donc le serveur ne recevrait ni fichier ni photo_recto.
            function toggleRequiredFields() {
                ...
                if (fileRecto) {
                    if (isUploadVisible) {
                        ...
                        // required est géré par le HTML/PHP selon la config
                    } else {
                        fileRecto.removeAttribute('required');
                        // Ne pas mettre disabled : le fichier doit pouvoir être envoyé si l'utilisateur
                        // a choisi un fichier puis affiché la section Caméra (cas rare).
                        fileRecto.removeAttribute('disabled');
                    }
                }
            ...
            function validateFile(file, maxSizeMB = 5) {
                ...
                if (!isTypeValid && !isJPEG) {
                    ...
                    return { 
                        valid: false, 
                        message: `Type de fichier non autorisé. Formats acceptés : JPG, JPEG, PNG, WEBP, PDF. Type détecté : ${file.type || 'inconnu'}. Extension : ${fileExtension}` 
                    };
                }
            ...
            if (fileRectoInput) {
                fileRectoInput.addEventListener('change', function(e) {
                    ...
                    const photoRectoHidden = document.getElementById('photoRectoData');
                    
                    if (!file) {
                        if (photoRectoHidden) photoRectoHidden.value = '';
                        const preview = document.getElementById('photoRectoPreview');
                    ...
                    // Envoyer aussi en base64 dans le champ caché pour garantir la réception côté serveur
                    // (un input file désactivé ou non envoyé ne contient pas le fichier)
                    if (photoRectoHidden && file.type && file.type.startsWith('image/')) {
                        const reader = new FileReader();
                        reader.onloadend = function() {
                            photoRectoHidden.value = reader.result;
                            console.log('✓ Recto copié en base64 pour envoi garanti');
                        };
                        reader.readAsDataURL(file);
                    } else if (photoRectoHidden && file.type === 'application/pdf') {
                        photoRectoHidden.value = '';
                    }
            ...
            if (fileVersoInput) {
                fileVersoInput.addEventListener('change', function(e) {
                    const file = e.target.files[0];
                    const photoVersoHidden = document.getElementById('photoVersoData');
                    
                    if (!file) {
                        if (photoVersoHidden) photoVersoHidden.value = '';
                        const preview = document.getElementById('photoVersoPreview');
                        if (preview) preview.innerHTML = '';
                        return;
                    }
                    ...
                    if (photoVersoHidden && file.type && file.type.startsWith('image/')) {
                        const reader = new FileReader();
                        reader.onloadend = function() {
                            photoVersoHidden.value = reader.result;
                            console.log('✓ Verso copié en base64 pour envoi garanti');
                        };
                        reader.readAsDataURL(file);
                    } else if (photoVersoHidden && file.type === 'application/pdf') {
                        photoVersoHidden.value = '';
                    }
            ...
            if (dateArriveeInput && dateDepartInput) {
                ...
                if (form) {
                    form.addEventListener('submit', function(e) {
                        ...
                        // Vérifier la pièce d'identité recto (requis) : au moins fichier OU photo
            ...
            // Fonction d'aperçu
            document.getElementById('showPreviewBtn').addEventListener('click', async function() {
                ...
                // Vérifier la pièce d'identité recto avant validation HTML5 : au moins fichier OU photo
                @if($formConfig->isRequired('piece_identite_recto'))
                ...
                if (fileRectoCheck && photoRectoDataElCheck) {
                    ...
                    // Si on a un fichier image mais pas encore de base64 (FileReader asynchrone), le remplir maintenant
                    // pour que l'aperçu ET la soumission aient bien photo_recto
                    if (hasFileRecto && !hasPhotoRecto && fileRectoCheck.files[0].type && fileRectoCheck.files[0].type.startsWith('image/')) {
                        await new Promise(function(resolve) {
                            const reader = new FileReader();
                            reader.onloadend = function() {
                                photoRectoDataElCheck.value = reader.result;
                                resolve();
                            };
                            reader.readAsDataURL(fileRectoCheck.files[0]);
                        });
                    }
                }
                @endif
                
                // Même chose pour le verso si requis
                ...
                if (fileVersoCheck && photoVersoDataElCheck && fileVersoCheck.files && fileVersoCheck.files.length > 0) {
                    const hasPhotoVerso = photoVersoDataElCheck.value && photoVersoDataElCheck.value.trim() !== '';
                    if (!hasPhotoVerso && fileVersoCheck.files[0].type && fileVersoCheck.files[0].type.startsWith('image/')) {
                        await new Promise(function(resolve) {
                            const reader = new FileReader();
                            reader.onloadend = function() {
                                photoVersoDataElCheck.value = reader.result;
                                resolve();
                            };
                            reader.readAsDataURL(fileVersoCheck.files[0]);
                        });
                    }
                }
                @endif
            ...
            // Fonction de confirmation et envoi (fetch + FormData pour garantir l'envoi du recto/verso)
            document.getElementById('confirmSubmitBtn').addEventListener('click', async function() {
                ...
                // Garantir que photo_recto est envoyé : si fichier image mais pas de base64, le lire maintenant
                const fr = document.getElementById('fileRecto');
                const photoRectoHidden = document.getElementById('photoRectoData');
                if (fr && fr.files && fr.files.length > 0 && fr.files[0].type && fr.files[0].type.startsWith('image/')) {
                    const hasPhotoRecto = formData.get('photo_recto') && String(formData.get('photo_recto')).trim().length > 0;
                    if (!hasPhotoRecto) {
                        await new Promise(function(resolve) {
                            const reader = new FileReader();
                            reader.onloadend = function() {
                                formData.set('photo_recto', reader.result);
                                if (photoRectoHidden) photoRectoHidden.value = reader.result;
                                resolve();
                            };
                            reader.readAsDataURL(fr.files[0]);
                        });
                    }
                }
                // Idem pour verso
                const fv = document.getElementById('fileVerso');
                const photoVersoHidden = document.getElementById('photoVersoData');
                if (fv && fv.files && fv.files.length > 0 && fv.files[0].type && fv.files[0].type.startsWith('image/')) {
                    const hasPhotoVerso = formData.get('photo_verso') && String(formData.get('photo_verso')).trim().length > 0;
                    if (!hasPhotoVerso) {
                        await new Promise(function(resolve) {
                            const reader = new FileReader();
                            reader.onloadend = function() {
                                formData.set('photo_verso', reader.result);
                                if (photoVersoHidden) photoVersoHidden.value = reader.result;
                                resolve();
                            };
                            reader.readAsDataURL(fv.files[0]);
                        });
                    }
                }
                
                // Ne garder qu'une seule source par face pour éviter dépassement post_max_size (fichier + base64 = double taille)
                if (formData.get('photo_recto')) {
                    formData.delete('piece_identite_recto');
                }
                if (formData.get('photo_verso')) {
                    formData.delete('piece_identite_verso');
                }
                ...
                try {
                    ...
                    // Afficher la réponse (page succès ou formulaire avec erreurs)
                    document.open();
                    document.write(html);
                    document.close();
                } catch (error) {
